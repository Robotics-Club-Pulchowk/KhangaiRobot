...
void init_GameField()
{
        ...
        gStateB_Data.centre = Vec2<float>(1750, 1500);
        gStateB_Data.upper_bounds = Vec2<float>(1950, 1800);
        gStateB_Data.lower_bounds = Vec2<float>(1650, 1400);
        ...
        gStateC_Data.centre = Vec2<float>(1650, 2400);
        gStateC_Data.upper_bounds = Vec2<float>(1850, 2700);
        ...
        gStateD_Data.centre = Vec2<float>(500, 2900);
        gStateD_Data.upper_bounds = Vec2<float>(700, 3100);
        gStateD_Data.lower_bounds = Vec2<float>(300, 2700);
        ...
        gStateE_Data.centre = Vec2<float>(700, 3900);
        gStateE_Data.upper_bounds = Vec2<float>(800, 4200);
        gStateE_Data.lower_bounds = Vec2<float>(500, 3500);
        gStateE_Data.last_limit = 1;
        gStateE_Data.ramping_factor = 0.001;
        gStateE_Data.first_limit = 1;

        // Defining the state : State F
        gStateF_Data.id = Field::FIELD_F;
        gStateF_Data.centre = Vec2<float>(1750, 4500);
        gStateF_Data.upper_bounds = Vec2<float>(1900, 4800);
        gStateF_Data.lower_bounds = Vec2<float>(1650, 4300);
        gStateF_Data.last_limit = 1;
        gStateF_Data.ramping_factor = 0.001;
        gStateF_Data.first_limit = 1;

        // Defining the state : State G
        gStateG_Data.id = Field::FIELD_G;
        gStateG_Data.centre = Vec2<float>(1650, 5500);
        gStateG_Data.upper_bounds = Vec2<float>(1900, 5800);
        gStateG_Data.lower_bounds = Vec2<float>(1600, 5300);
        ...
        gStateH_Data.upper_bounds = Vec2<float>(1300, 6500);
        gStateH_Data.lower_bounds = Vec2<float>(1100, 6000);
        ...
        gStateI_Data.centre = Vec2<float>(1250, 8700);
        gStateI_Data.upper_bounds = Vec2<float>(1350, 9000);
        gStateI_Data.lower_bounds = Vec2<float>(1125, 8500);
        gStateI_Data.last_limit = 0;
        gStateI_Data.ramping_factor = -0.01;
        gStateI_Data.first_limit = gStateH_Data.last_limit;
